...
const DASHBOARD_PASS =
  process.env.DASHBOARD_PASS || crypto.randomBytes(8).toString("hex");
...
app.ws("/control", (ws, req) => {
  ...
  ws.on("message", async (msg) => {
    try {
      ...
      if (!agent) {
        ws.send(
          JSON.stringify({ error: "Agent offline", requestId: cmd.requestId }),
        );
        return;
      }
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `🔐 Dashboard credentials: ${DASHBOARD_USER} / ${DASHBOARD_PASS}`,
  );
